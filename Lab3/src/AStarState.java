...
public class AStarState {
	...
	public Waypoint getMinOpenWaypoint() {
		if (openWaypoints.size() == 0)
			return null;
		Waypoint result = (Waypoint) openWaypoints.values().toArray()[0];
		for (Map.Entry<Location, Waypoint> entry : openWaypoints.entrySet())
			if (entry.getValue().getTotalCost() < result.getTotalCost())
	...
	public boolean addOpenWaypoint(Waypoint newWP) {
		...
		if (!openWaypoints.containsKey(newLoc)
				|| openWaypoints.get(newLoc).getPreviousCost() > newWP.getPreviousCost()) {
